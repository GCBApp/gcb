...
async function updateExchangeRates() {
  try {
    ...
    soap.createClient(url, async (err, client) => {
      ...
      for (const moneda of monedas.recordset) {
        ...
        client.TipoCambioFechaInicialMoneda(args, async (err, result) => {
          if (err) {
            console.error(`Error al obtener tipo de cambio para ${MON_moneda}:`, err);
            return;
          }
